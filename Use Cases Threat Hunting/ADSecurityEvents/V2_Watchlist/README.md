...
Custom Microsoft Sentinel analytic rules and hunting queries covering the full **Active Directory attack surface** based on the [AD Attack Architecture Map v1.1](https://kypvas.github.io/ad_attack_architecture/). Rules cover the complete kill chain: initial access → credential access → privilege escalation → lateral movement → persistence → domain dominance.
...
> **Companion repository:** [M365OfficeActivity](../M365OfficeActivity) — Microsoft 365 data-plane detections using `OfficeActivity`, `SigninLogs`, and `ThreatIntelligenceIndicator`. Together the two repositories cover the full on-premises AD + Microsoft 365 + Azure identity attack surface.
...
| Data connector | **Security Events via Legacy Agent** or **Windows Security Events via AMA** |
...
This package maps directly to the [AD Attack Architecture Map v1.1](https://kypvas.github.io/ad_attack_architecture/) kill chain:
...
| **HUNT-31** | **LDAP/SAMR Reconnaissance Burst** — BloodHound/SharpHound/AdFind/ldapsearch/`net.exe ... /domain` patterns; >=5 commands per actor | 1d | Recon detection (BloodHound enumeration) |
| **HUNT-32** | **KRBTGT Password Age Audit** — 4724/4723 against `krbtgt` over 180d, risk-scored P1/P2/OK by days since last reset | 180d | Golden Ticket exposure window hygiene |
| **HUNT-33** | **NTLM Relay to LDAP/LDAPS Indicator** — NTLM Type-3 logons to machine accounts where workstation name ≠ target (relay fingerprint) | 1d | PetitPotam/ntlmrelayx follow-on detection |
| **HUNT-34** | **First-Time Privileged Group Add per Actor** — 4728/4732/4756 to DA/EA/SA never seen for the actor in 30d | 7d | Anomalous privilege grant |
| **HUNT-35** | **AdminCount=1 Sprawl** — 5136 events setting `adminCount=1` on accounts outside tier-0 groups (SDProp persistence backdoor) | 30d | Persistence audit |
| **HUNT-36** | **Schema Modification** — 5136/5137 on `classSchema`/`attributeSchema` objects (extraordinarily rare; should never fire outside change-window) | 30d | Domain dominance / forest-level persistence |
...
### Option 1 — ARM Template (Recommended, deploys all 32 rules + 36 hunts)
...
### Staged deployment (recommended for production rollouts)

The template now includes a `NetworkAllowlist` watchlist and scalar function `ExcludeAllowlistedIPs_ADSec(ip:string)`. Every rule and hunt filters trusted IPs through this function (table-agnostic via `column_ifexists` for `IpAddress`, `SourceIP`, `ClientIP`, `IPAddress`). To roll out cleanly:

```bash
# 1) Provision watchlist + function + 36 hunts. Rules DISABLED.
az deployment group create -g <rg> \
  --template-file ARM-Template/ADSecurityEvents-Detections.json \
  --parameters workspaceName=<ws> enableAnalyticRules=false

# 2) Seed NetworkAllowlist (DCs, jump hosts, vuln scanners, backup servers).
#    Wait 5–15 min for watchlist propagation.
...
|---|---|---|
| `workspaceName` | `LAWSentinel` | Target Sentinel workspace |
| `location` | `[resourceGroup().location]` | Region |
| `enableAnalyticRules` | `true` | Staged-deploy switch |
| `watchlistAlias` | `NetworkAllowlist` | Watchlist alias |
| `functionAlias` | `ExcludeAllowlistedIPs_ADSec` | Function name (suffix avoids collisions with sibling packs) |
| `watchlistRawContent` | RFC1918 seed | Optional CSV body |

**Allowlist examples (vital for AD environments to avoid FPs):**

```csv
IPOrRange,Description,Owner,AddedDate
10.0.10.0/24,DC subnet,ADOps,2026-05-01
10.0.20.0/24,Tier0 jump hosts,SecOps,2026-05-01
10.0.99.10,Tenable scanner,VulnMgmt,2026-05-01
10.0.99.11,Backup server,InfraOps,2026-05-01
```

The accompanying `ARM-Template/Watchlists/` folder provides optional automation (Bicep + PowerShell runbook) to sync Entra Named Locations into `NetworkAllowlist` on a schedule.
