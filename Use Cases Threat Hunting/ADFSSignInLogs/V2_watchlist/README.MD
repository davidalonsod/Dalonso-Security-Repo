...
## 🛡️ Network Allowlist Watchlist (corporate IP filtering)

All 20 analytic rules in `Analytic-Rules/azuredeploy.json` now call a KQL
function `ExcludeAllowlistedIPs()` that filters out IPs/CIDRs/ranges defined
in the `NetworkAllowlist` Sentinel watchlist (synced from Entra Named
Locations + custom additions). This prevents corporate egress, VPN, and
known-good SaaS ranges from generating noise.

- KQL function and watchlist are deployed automatically by the ARM template
  (`Analytic-Rules/azuredeploy.json`).
- Scheduled sync from Entra Named Locations to the watchlist is provided
  separately. See [`Watchlists/README.md`](Watchlists/README.md).
- All Hunting Queries in [`HuntingQueries/`](HuntingQueries/) call the same
...
|---|---|---|
| `enableAnalyticRules` | `true` | Deploy all 20 rules in **enabled** state. Set to `false` to deploy disabled (review then enable per-rule). |
| `watchlistAlias` | `NetworkAllowlist` | Alias of the watchlist used by `ExcludeAllowlistedIPs`. |
| `functionAlias` | `ExcludeAllowlistedIPs` | KQL `savedSearches` alias for the filter function. |
| `watchlistRawContent` | seed CSV | Initial CSV (`SearchKey,Description,Type`) — usually overwritten by the scheduled sync from Named Locations. |

Example dry-run / staged deploy:
...
Ten complementary hunting queries are provided in
[`HuntingQueries/`](HuntingQueries/) covering long-range history, multi-table
correlations, and protocol/identity governance gaps. See
[`HuntingQueries/README.md`](HuntingQueries/README.md) for the full inventory.
...
```
ADFSSignInLogs-ThreatHunting/
├── ADFSSignInLogs-ThreatHunting.kql      ← 36 interactive hunting queries (KQL pack)
├── README.md                              ← This file
├── Analytic-Rules/
│   ├── azuredeploy.json                   ← ARM: 20 rules + NetworkAllowlist watchlist + ExcludeAllowlistedIPs function
│   ├── deploy-analytic-rules.ps1
│   └── rules/                             ← Per-rule YAML (deploy mode B)
├── Watchlists/
│   ├── azuredeploy-automation.bicep       ← Automation Account + SP-auth Runbook to sync Named Locations → NetworkAllowlist
│   ├── Sync-NamedLocationsToWatchlist.ps1
│   └── README.md
└── HuntingQueries/
    ├── HUNT-01..10_*.yaml                 ← Sentinel hunting query YAMLs
    └── README.md
```
