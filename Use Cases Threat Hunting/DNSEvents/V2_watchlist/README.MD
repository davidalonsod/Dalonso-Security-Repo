...
Suggested priority for max savings, least risk:

Exclude PTR/reverse lookups (Step 2) — usually 20–40% of DNS volume, near-zero security value.
Exclude your own internal domain suffixes for successful lookups.
Move the table to Auxiliary/Basic tier if no scheduled rule needs it hot.
Trim retention.
...
| `enableAnalyticRules` | `true` | If `false`, all 17 rules deploy in **disabled** state for staged rollout. |
