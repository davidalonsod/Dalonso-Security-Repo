<#
.SYNOPSIS
    Deploys 21 Netskope Internet Access Sentinel Analytic Rules + 19 Hunting Queries
...
.NOTES
    ...
    Version: 1.0.0
    Rules  : 21 Scheduled Analytic Rules — Netskope Internet Access (NetskopeEvents_CL)
    Queries: 19 Hunting Queries — deployed as savedSearches to Hunting blade
    Vendor : Netskope
    Required connectors: Netskope Data Connector (Content Hub)
#>
...
function Deploy-ViaArm {
    ...
    $deploymentName = "NK-IA-Rules-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
    ...
    if ($result.ProvisioningState -eq 'Succeeded') {
        ...
        Write-Host "  Microsoft Sentinel → Hunting → My Queries" -ForegroundColor Gray
    }
    else {
...
function Deploy-ViaYaml {
    ...
    foreach ($file in $yamlFiles) {
        ...
        try {
            # Parse YAML via Python (requires pyyaml)
            $pythonExe = 'python'
            $parseScript = @"
...
Write-Header 'Netskope Internet Access Sentinel Analytic Rules Deployer'
...
Write-Success 'Hunting queries visible in Microsoft Sentinel → Hunting → My Queries.'
Write-Host ''
