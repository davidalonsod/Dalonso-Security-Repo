<#
.SYNOPSIS
    Generate the Foundry / Application Insights Sentinel ARM template from
    analytic-rule, hunting-query, and watchlist source files.
...
.DESCRIPTION
    ...
    The content reads the real Foundry / Agent Service trace shape that
    lands in Application Insights: structural spans in AppDependencies and
    protected prompts, responses, and tool payloads in AppGenAIContent.
...
.NOTES
    Agent365/Foundry-AppInsights/Deploy/New-FoundryArmTemplate.ps1
...
if (Test-Path $analyticDir) {
    Get-ChildItem -Path $analyticDir -Filter *.yaml | ForEach-Object {
        ...
        if ($techniques -contains 'T1199' -and $tactics -notcontains 'InitialAccess') {
            throw "Invalid MITRE mapping in $($_.Name): T1199 requires the InitialAccess tactic."
        }
        if ($techniques -contains 'T1592' -and $tactics -notcontains 'Reconnaissance') {
            throw "Invalid MITRE mapping in $($_.Name): T1592 requires the Reconnaissance tactic."
        }
        ...
        $ruleEnabled = if ($null -ne $y.enabled -and -not [bool]$y.enabled) {
            $false
        }
        else {
...
$template = [ordered]@{
    ...
    metadata       = @{
        description = 'Foundry / Application Insights - Microsoft Sentinel analytics, hunts, and watchlists over AppDependencies gen_ai.* spans and protected AppGenAIContent.'
        author      = 'Sentinel-As-Code / Agent365 Foundry-AppInsights bundle'
    }
    parameters     = [ordered]@{
        workspaceName       = @{
            type     = 'string'
            metadata = @{ description = 'Name of the Log Analytics workspace where Microsoft Sentinel is enabled and the Foundry Application Insights telemetry is exported (e.g. LAWSentinel).' }
        }
        enableAnalyticRules = @{
            type         = 'bool'
            defaultValue = $true
        ...
        enableWatchlist     = @{
            type         = 'bool'
            defaultValue = $true
            metadata     = @{ description = 'Set to false to skip all bundled watchlists if you manage them separately. Allowlist-based rules depend on their corresponding watchlists.' }
        }
    }
    variables      = @{}
...
$paramsJson = ($params | ConvertTo-Json -Depth 8) -replace "`r`n", "`n"
