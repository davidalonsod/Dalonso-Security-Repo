...
.SYNOPSIS
    ...
    the YAML hunting-query source files.
...
.DESCRIPTION
    ...
    Both content types read the real Foundry / Agent Service trace shape
    that lands in Application Insights and is exported to the Sentinel
    workspace: spans in the AppDependencies table, property bag in
    Properties, with gen_ai.* and microsoft.foundry.content_filter.results
    keys.
...
$template = [ordered]@{
    ...
    metadata       = @{
        description = 'Foundry / Application Insights - Microsoft Sentinel analytic rules and hunting queries (read AppDependencies / Properties / gen_ai.*).'
    ...
    parameters     = [ordered]@{
        ...
        enableWatchlist     = @{
            type         = 'bool'
            defaultValue = $true
            metadata     = @{ description = 'Set to false to skip deploying the FoundryTrustedToolSources watchlist (e.g. if you manage it separately). The untrusted-tool-source rule depends on it.' }
        }
    }
    variables      = @{}
