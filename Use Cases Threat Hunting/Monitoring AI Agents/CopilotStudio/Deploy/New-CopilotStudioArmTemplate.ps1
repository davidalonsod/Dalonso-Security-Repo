    ...
    workspace:
      ...
      - Connector / action calls in the AppDependencies table
        ...
        Studio") with Name / Target / Success / DurationMs.
            - Environment-level InvokeAgent / ExecuteTool / OutputMessages spans
                in AppDependencies with gen_ai.agent.*, gen_ai.tool.*, and shared
                OperationId trace correlation.

    Analytic rules are deployed as Microsoft.SecurityInsights/alertRules of
        kind Scheduled. Rules enabled in YAML are bound to the deployment
        parameter "enableAnalyticRules"; rules disabled in YAML remain disabled.
...
.NOTES
    Monitoring AI Agents/CopilotStudio/Deploy/New-CopilotStudioArmTemplate.ps1
...
if (Test-Path $analyticDir) {
    Get-ChildItem -Path $analyticDir -Filter *.yaml | ForEach-Object {
        ...
        $ruleEnabled = if ($null -eq $y.enabled -or [bool]$y.enabled) {
...
$template = [ordered]@{
    ...
    metadata       = @{
        description = 'Copilot Studio / Application Insights - Microsoft Sentinel analytic rules and hunting queries for agent events, connector calls, and environment-level InvokeAgent / ExecuteTool spans.'
